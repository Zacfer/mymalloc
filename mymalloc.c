...
int myinit(int *array, int size) {
    ...
    // The initialised array's header information needs 2 ints + 3 ints minimum for the first block
    if(size > 4) {
        array[0] = size;
        
        // Stores the index of the very first block that will be allocated, NO_NEXT_FLAG at initialisation
        array[1] = NO_NEXT_FLAG;
...
int mydispose(int *array) {
    ...
    if(array[1] != NO_NEXT_FLAG) return 0;
...
int myfree(int *array, int * block) {
    ...
    int j, previousBlock = 1, currentBlock = array[previousBlock];
 ...
 * to that block. Implementation of a linked list of blocks inside the array, using a best-fit algorithm to allocate. 
...
int * mymalloc(int *array, int size) {
    ...
    int blockSize = size + 2, *result = (int *)0, currentBlock = 1, nextBlock = NO_NEXT_FLAG, newBlock = 0, availableSpace = 0;
    ...
    while(1) {
        ...
        // If 'currentBlock' is 1 (which is the index to the array's header information for the first block), then 'newBlock' should be placed at 2
        // else at the index right after the 'currentBlock'
        newBlock = (currentBlock == 1)?2:(currentBlock + 2 + array[currentBlock + 1]);
        ...
        if(selectedAvailableSpace < availableSpace) {
